...
def find_intersection_two_tensors(A, B):
...
class NRGNN:
    ...
    def fit(self, features, adj, labels, idx_train, idx_val):
        ...
        self.estimator = EstimateAdj(edge_index, features, features.shape[1], args, idx_train, device=self.device).to(
            self.device)
...
#         predictor_weights = torch.cat([torch.ones([edge_index.shape[1]], device=self.device), predictor_weights], dim=0)
        ...
        # edge similarity
        self.estimator.edge_sim(edge_index,features)
        print('%'*20)
...
#             unlabel_edge_index, idx_add = self.get_model_edge(pred)
#             print(self.idx_add)
#             if unlabel_edge_index.nelement() > 0:
#                 print('cat')
#                 self.unlabel_edge_index = find_intersection_two_tensors(self.unlabel_edge_index, unlabel_edge_index)
# #                 self.unlabel_edge_index = torch.unique(torch.cat([self.unlabel_edge_index, unlabel_edge_index], dim=1), dim=0)
# #                 self.unlabel_edge_index = torch.cat([self.unlabel_edge_index, unlabel_edge_index], dim=1)
#                 self.idx_add = torch.unique(torch.cat([self.idx_add, idx_add]))
#                 self.idx_add = find_intersection_two_tensors(self.idx_add, idx_add)
    
#             else:
#                 self.unlabel_edge_index = unlabel_edge_index
...
class EstimateAdj(nn.Module):
    ...
    def edge_sim(self, edge_index,features):
        import matplotlib.pyplot as plt
#         x0 = features[edge_index[0]]
#         x1 = features[edge_index[1]]
#         sim = torch.div(torch.matmul(x0, x1.T), x0.norm() * x1.norm(dim=1))
#         print(sim)
#         sim[sim < 0.5] = 0.0
        sim = dict()
        for i in range(edge_index.shape[1]):
            x0 = features[edge_index[0][i]]
            x1 = features[edge_index[1][i]]
            s = (torch.div(torch.matmul(x0, x1.T), x0.norm() * x1.norm())).item()
            s = round(s,2)
            if s in sim.keys():
                sim[s] += 1
            else:
                sim[s] = 0
        print(dict(sorted(sim.items())))
#         plt.bar(sim.keys(), sim.values(), 1.0, color='g')
        n = 0
        for k in sim.keys():
            if k > 0.5:
                n += sim[k]

        mylist = [key for key, val in sim.items() for _ in range(val)]
        plt.hist(mylist, bins=30)
        plt.savefig('sim.png')
        plt.show()
        print("larger than 0.5",n, "total:", edge_index.shape[1])

            
#         print(sorted(sim, reverse = True)[:10],len(sim))
