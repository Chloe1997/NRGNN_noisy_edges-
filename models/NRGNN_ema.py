...
class NRGNN:
    ...
    def update_ema_variables(self, model, ema_model, epoch, alpha = 0.999):
        # Use the true average until the exponential average is more correct
        alpha = min(1 - 1 / (epoch + 1), alpha)
#         if epoch % 100:
#             print(alpha)
        for ema_param, param in zip(ema_model.parameters(), model.parameters()):
            ema_param.data = ema_param.data.mul(alpha).add(param.data.mul(1 - alpha))
#             ema_param.data = ema_param.data.mul(1 - alpha).add(param.data.mul(alpha))
    ...
    def train(self, epoch, features, edge_index, idx_train, idx_val):
        ...
        total_loss = loss_gcn + loss_pred + self.args.alpha * rec_loss + self.args.beta * loss_add + loss_label_smooth * 2
        #         total_loss = loss_gcn + loss_pred + self.args.alpha * rec_loss  + self.args.beta * loss_add
#         total_loss = loss_gcn  + self.args.alpha * rec_loss + self.args.beta * loss_add + loss_pred

        total_loss.backward()

        self.optimizer.step()

        # update for predictor
        self.update_ema_variables(self.model, self.predictor, epoch)
        ...
        if acc_pred_val > self.best_acc_pred_val:
            ...
            # print(unlabel_edge_index)
            # if unlabel_edge_index.nelement() > 0:
            #     print('cat')
            #     # self.unlabel_edge_index = torch.unique(torch.cat([self.unlabel_edge_index, unlabel_edge_index], dim=1))
